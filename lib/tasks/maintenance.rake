...
namespace :maintenance do
  ...
  task update_cache_counters: :environment do
    ...
    Dir.glob(Rails.root.join('app/models/**/*')).sort.each { |model| require model if File.file?(model) }
    ...
    ActiveRecord::Base.descendants.each do |model|
      ...
      models_to_update[model] = cache_counters if cache_counters.size.positive?
    end
